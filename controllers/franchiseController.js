...
const Order = require("../models/franchise-models/customerorder");
...
const handleCreateFranchise = async (req, res) => {
  try {
    ...
    if (existingFranchise) {
      return res
        .status(400)
        .json({ message: "Franchise already exists with this email." });
    }
    ...
    if (contactNumber) {
      return res
        .status(400)
        .json({
          message: "Franchise already exists with this contact Number.",
        });
    }
...
const handleAssignProductsToFranchise = async (req, res) => {
  try {
    ...
    for (const product of products) {
      ...
      if (productFound.stock < quantity) {
        return res
          .status(200)
          .json({
            message: `Product: ${productFound.name} has only ${productFound.stock} quantity in Stock.`,
          });
      }
    }
    ...
    for (const product of products) {
      ...
      if (productFound.stock < quantity) {
        return res
          .status(200)
          .json({
            message: `Product with productId: ${productId} has only ${productFound.stock} quantity in Stock.`,
          });
      }

      // Add/Update product in the franchise's inventory
      const existingInventoryItem = inventory.products.find(
    ...
    return res
      .status(200)
      .json({
        message: "Products assigned successfully to franchise",
        franchiseId,
        assignedProducts,
        totalPrice,
      });
...
const generateUniqueFranchiseOrderNumber = async () => {
  ...
  while (!isUnique) {
    ...
    const existingOrder = await FranchiseOrder.findOne({
...
const handleGetFranchiesInventory = async (req, res) => {
  try {
      ...
      inventory.products.map(async (item) => {
        ...
        if (product) {
          return {
            ...
            sellprice: product.sellprice, // Add sell price from the Product model
            gst: product.gst, // Add GST from the Product model
          };
        }
    ...
    return res
      .status(500)
      .json({ message: "Error finding Inventory", error: e.message });
  }
};
// const handleGetFranchiesInventory = async (req, res) => {
//     try {
...
const handleRemoveProductFromFranchiseInventory = async (req, res) => {
  try {
    ...
    if (!franchiseId || !productId) {
      return res
        .status(400)
        .json({ message: "Please provide both Franchise ID and Product ID" });
    }
    ...
    if (!franchise) {
      return res
        .status(404)
        .json({
          message: "Franchise not found with the provided Franchise ID",
        });
    }
    ...
    if (!inventory) {
      return res
        .status(404)
        .json({ message: "Inventory not found for the given franchise" });
    }
    ...
    if (productIndex === -1) {
      return res
        .status(404)
        .json({ message: "Product not found in the franchise inventory" });
    }
...
const handleLoginFranchise = async (req, res) => {
  try {
    ...
    if (!franchiseId || !password) {
      return res
        .status(400)
        .json({ message: "Please provide both franchiseId and password." });
    }

    // Find the franchise by email
    const franchise = await Franchise.findOne({ franchiseId });
    if (!franchise) {
      return res.status(401).json({ message: "Invalid franchiseId." });
    }
    ...
    if (isPasswordMatch) {
      ...
      return res.json({
        ...
        email: franchise.email,
        contactno: franchise.contactInfo,
      ...
      return res
        .status(404)
        .json({ message: "Invalid franchiseId or password." });
    }
  } catch (error) {
...
const handleCalculateTotalBill = async (req, res) => {
  try {
    ...
    if (!franchiseId || !products) {
      return res
        .status(400)
        .json({ message: "Please provide both Franchise ID and Products." });
    }
    ...
    for (let product of products) {
      ...
      if (!productId || !quantity) {
        return res
          ...
          .json({
            message:
              "Please provide both Product ID and Quantity for each product.",
          });
      }
      ...
      if (productFound.quantity < quantity) {
        return res
          .status(200)
          .json({
            message: `Product with productId: ${productId} has only ${productFound.quantity} quantity in Stock.`,
          });
      }
    }
    ...
    for (let product of products) {
      const { productId, quantity } = product;
      const productFound = inventory.products.find(
        (item) => item.productId.toString() === productId
      );
    ...
    return res
      .status(200)
      .json({ message: "Total bill calculated successfully", totalPrice });
  } catch (error) {
  ...
  try {
    ...
    while (currentUser.parentSponsorId) {
      ...
      if (ancestor.mySponsorId === rcvdSponsorId) {
        ...
        // Check if current user (purchaser) is in the left or right subtree of ancestor
        if (isInLeftTree) {
          ancestorBVPoints.directBV.leftBV += totalBvPoints;
        } else {
          ancestorBVPoints.directBV.rightBV += totalBvPoints;
        }
        await ancestorBVPoints.save();
      }
...
const handleGetFranchiseOrders = async (req, res) => {
  try {
    ...
    const franchiseOrders = await FranchiseOrder.find({
      "franchiseDetails.franchiseId": franchiseId,
    });
    if (!franchiseOrders) {
...
const handleGetAllOrdersCreatedByFranchise = async (req, res) => {
  try {
    ...
    const franchiseOrders = await UserOrder.find({
      "franchiseDetails.franchiseId": franchiseId,
    }).populate("userDetails.user", "mySponsorId name email");
...
// order create for individual customer

// const customerorder = async (req, res) => {
//   try {
//     const { franchiseId, fullName, phoneNumber, address, products, payment } =
//       req.body;

//     if (!franchiseId || !fullName || !phoneNumber || !address || !payment || !products?.length) {
...
const customerorder = async (req, res) => {
  try {
    ...
    if (!inventory) {
      return res
        .status(404)
        .json({ message: "Inventory not found for this franchise" });
    }
    ...
    for (let item of products) {
      ...
      totalPrice += inventoryProduct.price * quantity;
    }
